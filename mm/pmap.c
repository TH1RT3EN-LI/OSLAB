...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    for (offset = 0; offset < size; offset += BY2PG) {
        Pte *pte = boot_pgdir_walk(pgdir, va + offset, 1); 
        if (pte == NULL) {
            panic("boot_map_segment: fail to create page table\n");
        }

        *pte = (pa + offset) | perm | PTE_V; 
    }
...
page_init(void)
{
    size_t i, freecount = 0;
    struct Page *pp;
    u_long cutoff;

    LIST_INIT(&page_free_list);
    freemem = ROUND(freemem, BY2PG);
    cutoff = PADDR(freemem); 

    for (i = 0; i < npage; i++) {
        pp = &pages[i];
        if (page2pa(pp) < cutoff) {
            pp->pp_ref = 1;
        } else {
            pp->pp_ref = 0;
