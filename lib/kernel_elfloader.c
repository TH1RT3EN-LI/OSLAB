	...
	if (size < 4 || !is_elf_format(binary)) {
	 ...
	 /* Your task here!  */
        ...
        /* Hint: Call the callback function you have achieved before. */
					/* Map the segment into the user space. */
					// args[0] is the virtual address where the segment should be mapped
					// args[1] is the size of the segment
					// args[2] is the pointer to the binary data to be loaded into memory
					// args[3] is the size of the binary data to be loaded
					// args[4] is the user data pointer which point to a certain env(process)
